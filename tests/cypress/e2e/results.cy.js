...
const config = {
	id: 'search', // id of controller in index.js
	...
	selectors: {
		sidebar: {
			...
			facetWrapper: '.ss__sidebar .ss__facet', // facet wrapper
			facetTitle: '.ss__block__header .ss__title', // facet title, should contain facet.label innerText
			facetCollapseButton: '.ss__block__header.ss__pointer', // facet collapse button, should contain onClick to toggle collapse
			facetOpen: '.ss__expanded', // facet open state class
			facetCollapsed: '.ss__collapsed', // facet closed state class
			facetOption: '.ss__block__section div:not(.ss__show-more) a', // facet option, should be <a> element or element handling onClick
			showMoreButton: '.ss__show-more__link', // facet show more button, should contain onClick
			searchWithinInput: '', // facet search within, should be <input> element
			summaryWrapper: '.ss__summary .ss__list', // filter summary wrapper
			appliedFacetRemoveButton: '.ss__list__option:not(.ss__list__option--clear) .ss__list__link', // filter summary - a filter's remove button onClick and/or <a> element
			removeAllFacetsButton: '.ss__list__option--clear .ss__list__link', // filter summary clear all button
		},
		sortBy: {
		...
		pagination: {
			infinite: false,
			loadMoreButton: '',
			prev: '.ss__pagination .ss__pagination__previous', // pagination previous
			page: '.ss__pagination .ss__pagination__column', // pagination page
			next: '.ss__pagination .ss__pagination__next', // pagination next
		},
		results: {
			productWrapper: '.ss__content .ss__results .ss__result', // single result wrapper
		},
	},
};
	...
	describe(`${page.id || _i}`, () => {
		describe('Setup', () => {
			it('adds snap bundle to search page', function () {
				...
				cy.snapController(config.id).then(({ store }) => {
		...
		describe('Results Tests', () => {
			beforeEach('reset', function () {
				cy.snapController(config.id).then(({ store, urlManager }) => {
					// preserve initial query
					const query = urlManager.state.query;
					urlManager.set(query ? { query } : {}).go();

					cy.snapController(config.id).then(({ urlManager }) => {
